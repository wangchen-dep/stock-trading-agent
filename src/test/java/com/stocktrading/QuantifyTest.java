...
//            String endDate = "2020-06-30";
            ...
            String path = startDate + "_" + endDate;

            // 文件路径配置：定义各个阶段数据的存储位置
            // rawDataPath: 原始股票数据（OHLCV - 开高低收成交量）
            String rawDataPath = String.format("%s/%s.csv", "data/raw" + path, symbol);

            // featuresPath: 处理后的特征数据（包含技术指标）
            String featuresPath = String.format("%s/%s_features.csv", "data/features" + path, symbol);

            // modelPath: 训练好的机器学习模型文件
            String modelPath = String.format("%s/%s_model.model", "models", symbol);

            // resultsPath: 回测结果输出文件
            String resultsPath = String.format("%s/%s_backtest.csv", "results" + path, symbol);
            ...
            log.info("目标：计算技术指标，将原始价格数据转换为机器学习特征");
            log.info("计算的指标包括：");
            log.info("- 移动平均线（MA5, MA10, MA20, MA50）");
            log.info("- 指数移动平均线（EMA12, EMA26）");
            log.info("- 相对强弱指标（RSI）");
            log.info("- MACD（MACD线、信号线、柱状图）");
            log.info("- 布林带（上轨、中轨、下轨）");
            log.info("- 成交量指标、动量指标等");
